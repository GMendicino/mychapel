...
export const isPublished = async (tourId: string): Promise<boolean> => {
    const token = getAuthToken();
    if (!token) throw new Error("Not authenticated");

    const response = await fetch(`/is-published?tourId=${tourId}`, {
        headers: { "Authorization": `Bearer ${token}` }
    });
...
export const unpublishTour = async (tourId: string): Promise<boolean> => {
    const token = getAuthToken();
    if (!token) throw new Error("Not authenticated");

    const response = await fetch("/unpublish-tour", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
...
export const publishTour = async (project: Project): Promise<boolean> => {
    const token = getAuthToken();
    if (!token) throw new Error("Not authenticated");
    ...
    const response = await fetch("/publish-tour", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
